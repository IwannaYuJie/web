...
## [2026-09-23] - 仙侠风格加料：红梅、仙鹤与滚动入场

- 首屏新增墨梅一枝（折角枯枝 + 飞白笔触 + 红梅花苞）与飘落的花瓣；两只丹顶鹤扇翅掠过，替换原来的简笔飞鸟。
- 首屏视差：滚动时远近三层山错速移动，鼠标移动时明月与梅枝轻微偏移；诗句每 8 秒轮换一联。
- 「镇阁之作」挂轴加上装裱绫边（石青、黛紫、赭石），上宽下窄，内衬泥金细线。
- 内页页头左上角加小枝红梅与零星落花。
- 滚动入场：`Layout` 用 IntersectionObserver 给 `.reveal` 区块与 `.section-h` 标题加 `.in`；毛笔标题下方随之刷出一道淡金笔触。
- 文章卷面上下加回纹带，首段首字下沉为朱砂毛笔字。
- 新增配色令牌 `--blossom`、`--blossom-soft`、`--silk-1…3`，夜间各有对应色；所有动效均尊重 `prefers-reduced-motion`。
- 验证：lint、134 项测试、构建通过；1440px 桌面、390px 手机（无横向溢出）与夜间模式截图检查。

 ---

## [2026-09-23] - 全站改版：仙侠风格「云深处」

### 视觉与交互

- 设计系统整体重写：宣纸 / 黛墨 / 朱砂 / 泥金配色，夜间跟随系统切换为月夜配色；字体改为 Ma Shan Zheng（毛笔标题）、ZCOOL XiaoWei（雅致副标）、Noto Serif SC（正文）、Cormorant Garamond（数字）。
- 新增 `src/components/xian/` 装饰组件：水墨远山、祥云、朱砂印、明月、飞鸟、全站云雾与灵光氛围层，以及内页统一页头 `PageHeader`。
- 栏目改用雅称并保留白话提示：山门、藏经阁、万象、百工坊、游仙境、法宝阁、洞府、执笔；路由不变。
- 首页：远山明月首屏 + 竖排毛笔题字、「今日开卷」、挂轴式「镇阁之作」、中文序号文章列表、竖排菜单与侧栏箴言。
- 文章页：卷面排版、祥云引首的小节标题、砚台色代码块、表格样式、文末落款与印章、随阅读高亮的目录、带标题的上下篇导航。
- 藏经阁改为竖排年份的时间线；万象改为按篇数缩放的签引星图与门类卡；洞府新增月晕头像、竖排心法、修行纪事；法宝阁改为符箓式卡片，工具页统一页头；游仙境改为印章卡片；后台登录、文章列表与 9 月 22 日新增的资讯草稿面板同步换装（草稿逻辑未改）。
- 移除顶部跑马灯、点击冒 emoji 特效和页脚里随机增长的「累计访客」假数字；页脚改为对联 + 真实的建站天数。

### 结构与修复

- 创意工坊的预览弹窗、风格卡片与取色提示抽成共享组件（`components/creative/`），首页与百工坊页不再各维护一份。
- 工具清单集中到 `src/data/tools.js`，工具箱首页与各工具页头共用。
- 修复 Markdown 行内代码被渲染成代码块的问题（react-markdown v10 不再传 `inline`，改由 `pre` 接管代码块）。
- 修复开发环境下正文标题锚点与目录错位的问题：锚点改按源码行号分配，不再依赖渲染计数（StrictMode 重复渲染会让计数跳位）。
- 修复页面切换动画保留 `filter` 导致弹窗无法全屏覆盖的问题；`main` 不再建立层叠上下文。
- Tailwind 的 `primary` / 新增 `surface` 支持透明度写法，工具页与后台去掉硬编码的 `bg-white`，深色模式下不再发白。
- 新增 `src/utils/xianText.js`（中文数字、年份、月份、时辰、分类配色）及 7 项测试。
- 上线后修正：后台登录页外层误用带限宽的 `.xadmin`，页头与登录框偏在左侧；ZCOOL XiaoWei 的「回」字字形是实心方块，改由只覆盖 U+56DE 的 `Elegant Glyph Fix` 字体规则回退到系统宋体 / 黑体（逐字检测了站内用到的 2200 余个汉字，只有这一个字有问题）。

### 验证

- lint、typecheck、生产构建通过；测试 123 项全部通过（新增 7 项）。
- 浏览器验收了桌面 1440px 下的首页、文章页、藏经阁、万象、洞府、百工坊（含预览弹窗、键盘切换与 Esc 关闭）、游仙境（进入 2048 对局）、法宝阁、JSON 与精灵图工具、后台登录页。手机 390px 下上述页面均无横向溢出，竖排菜单可用；深色模式首页已检查。

 ---

## [2026-09-22] - 私有 AI 资讯草稿箱（待部署验收）

### 功能与边界

- 文章管理新增「资讯草稿」页签，支持 `/admin/articles?tab=news` 直达，沿用管理员密钥登录。
- 展示官方来源、原文链接与日期、AI 草稿标记、采集进度与每日剩余额度；支持 Markdown 预览、编辑标题/摘要/正文/标签、保存、丢弃及确认后发布为公开文章。
- 发布分类为 `AI 资讯`；旧文章编辑器同时兼容已有自定义分类，发布后可以继续维护文章。
- 新增真正的 Cloudflare Python Worker + Workflows，通过 Workers AI 整理 Cloudflare Blog、OpenAI News 官方 RSS；每天 UTC+8 09:00 自动采集，也可手动启动，每天最多保存 3 条新草稿。
- 私有草稿独立存储；来源去重覆盖已发布与已丢弃记录。独立采集凭据不能发布文章，管理员发布重试复用同一篇文章，避免重复发布。
- 手动采集的 202 按「已排队」处理，前端保留最长 3 分钟的启动跟踪；仅活动任务有限轮询，错误或长时间等待可手动刷新。
- Markdown 预览增加局部错误边界，模块暂时加载失败时仍可编辑和保存草稿。Vite 接口测试改用独立临时缓存与草稿文件，避免并发测试覆盖浏览器开发服务器的依赖缓存。
- VPS/Vite 接入共用草稿处理器；旧 Pages 备用 API 未增加草稿接口。仓库备份脚本增加草稿文件，首次上线需同步已安装脚本。

### 文档与验收状态

- 新增 [NEWS_DRAFTS.md](NEWS_DRAFTS.md)、[Python 采集器说明](../workers/news-drafts/README.md)，同步 README 与架构说明。
- 前端定向 lint、类型检查和差异空白检查已通过；全量自动化、构建与浏览器验收由主任务继续执行，最终结果补记于专题文档。
- **当前状态：工作树实现完成并继续验收；尚未完成 Cloudflare/VPS 线上部署与真实采集验收。本条不代表功能已上线。**

## [2026-09-05] - 发布 GPT-6 Astra 解读文章

- 新增《GPT-6 Astra 解读：当 AI 开始接手一整件事》，作者 Doro，分类「架构设计」，预计阅读 7 分钟；正文解释基本规格、异步工具调用、中途追加要求、与 GPT-5.6 Sol 的价格对比及任务验收方法。
- 源稿：`content/articles/2026-09-05-gpt-6-astra.md`。依据当日实际查阅的 OpenAI 官方模型页与功能文档编写，正文附来源链接，明确区分官方说明、假设计算与作者分析。
- 已通过正式生产 API 发布，文章 ID **17**，地址：<https://jumaomaomaoju.cn/article/17>。仅新增文章数据，无应用部署。
- 发布前完成源稿 dry-run、同题去重、价格算例复算，并在本机临时目录保存全部 16 篇旧文章的完整快照（不提交仓库）。
- 发布后列表与详情 API 均返回 HTTP 200；标题、摘要、日期、分类、阅读时间、作者、标签与正文八个字段和本地源稿一致；总数增至 17 篇，原 16 篇逐对象比对保持一致。
- 真实浏览器确认文章标题、完整正文、价格表、六节目录、来源链接及标签已正常展示。正文 SHA-256：`e86dc823b4d49922155a6138f4cc4f59ab6ef5354f1b6b8cbcd38b173724d73e`。
...
- **状态：本地完成并提交；2026-09-05 已完成服务器发布脚本同步与回滚备份，发布准备及验收入口见部署说明。**
