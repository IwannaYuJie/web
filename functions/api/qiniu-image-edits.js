/**
 * Cloudflare Pages Functions - 七牛图生图 API 代理
 * 将前端请求安全地转发到 https://api.qnaigc.com/v1/images/edits
 */
...
export async function onRequest(context) {
  ...
  if (!finalPayload.image) {
    return new Response(JSON.stringify({ error: 'image 参数不能为空' }), {
      status: 400,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    })
  }
  ...
  try {
    ...
    if (result.response.ok && result.body?.data?.length) {
      waitUntil(sendSuccessEmail(env, { images: result.body.data, prompt, source: 'qiniu-edit' }))
    } else {
      responseBody = shapeQiniuErrorBody(result.body, '七牛图生图调用失败')
      waitUntil(sendFailureEmail(env, { error: responseBody.message, prompt, source: 'qiniu-edit' }))
    }
    ...
    waitUntil(sendFailureEmail(env, { error: error.message, prompt, source: 'qiniu-edit' }))
